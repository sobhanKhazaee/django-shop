    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if user:
                ...
                email_sent = send_email("فعالسازی حساب کاربری", new_user.email, {"user": new_user}, "email/active_account.html")
                if email_sent:  # فرض بر این که send_email مقدار True/False برمی‌گرداند
                    new_user.save()
                    return redirect(reverse('login_page'))
                else:
                    register_form.add_error(None, "ارسال ایمیل با خطا مواجه شد، لطفاً دوباره تلاش کنید.")
        else:
            context = {
                'register_form': register_form
            }
