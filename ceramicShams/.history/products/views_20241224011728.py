...
class sortByCatView(ListView):
    template_name = template_name = 'products/sortedByCat.html'
    model = Product
    context_object_name = 'products'
    

    def get_queryset(self):
        # دریافت دسته‌بندی بر اساس cat_id و محصولات مرتبط
        self.cat = get_object_or_404(Category, id=self.kwargs['cat_id'])
        return self.cat.products.all()

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['cats'] = Category.objects.all()  # تمام دسته‌بندی‌ها
        context['cat'] = self.cat  # دسته‌بندی انتخاب‌شده
        return context
