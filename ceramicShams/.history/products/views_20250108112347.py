...
from .models import Product, Category, Brand,ProductsVisit
...
class productDetailsView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        seen = ProductsVisit.objects.filter(ip__iexact = user_ip , product = self.object.id).exists()
        if not seen:
            new_visit = ProductsVisit(ip = user_ip , user_id = user_id , product_id = self.object.id )
            new_visit.save()
