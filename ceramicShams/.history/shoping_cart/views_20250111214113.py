...
def remove_from_cart(request):
    ...
    try:
        cart = Cart.objects.get(user=request.user, payment=False)  # سبد خرید کاربر
    except Cart.DoesNotExist:
        return JsonResponse({'error': 'سبد خرید پیدا نشد'}, status=404)

    order = Order.objects.filter(cart=cart, product_id=product_id).first()  # سفارش در سبد خرید

    if not order:
        return JsonResponse({'error': 'سفارش مورد نظر یافت نشد'}, status=404)

    order.delete()
    return JsonResponse({'success': 'سفارش مورد نظر از سبد خرید شما حذف شد'})
