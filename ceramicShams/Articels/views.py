...
def submit_comment(request):
    if request.method == 'POST':
        ...
        parent_id = request.POST.get("parent_id")  # دریافت parent_id برای پاسخ‌ها
        ...
        if article_id:
            if comment_text:
                ...
                if parent_id:
                    try:
                        parent_comment = ArticelComments.objects.get(id=parent_id)
                    except ArticelComments.DoesNotExist:
                        return JsonResponse({'success': False, 'message': 'نظر والد یافت نشد.'})

                comment = ArticelComments(
                    article=article,
                    user=request.user,
                    parent=parent_comment,  # تنظیم نظر والد
                    comment=comment_text
                )
                comment.save()

                # دریافت نظرات و پاسخ‌ها برای رندر
                context = {
                    'comments': ArticelComments.objects.filter(
                        article_id=article_id, parent=None).order_by('-created_at').prefetch_related('articelcomments_set'),
                }
                # رندر کردن HTML برای نظرات و پاسخ‌ها
                html = render_to_string('includes/articel_comment_partial.html', context)

                return JsonResponse({
                    'success': True,
                    'html': html,  # بازگشت HTML جدید
                })
            else:
